...
from scripts.v02_125_e08_remaining_sidecar_architecture_reselection import (
...
def test_v02_125_selects_production_worker_supervision() -> None:
    result = select_slice()

    assert result["decision"] == "select_production_worker_supervision"
    assert result["selected_slice"]["slice_id"] == "production_worker_supervision"
    assert result["next_version"] == "v0.2.126_e08_production_worker_supervision"
    assert result["first_design"] == "docs/current-design/design_v0_2_126_e08_production_worker_supervision.md"


def test_v02_125_excludes_completed_worker_task_kind_coverage() -> None:
    result = select_slice()
    ...
    assert result["invariants"]["completed_builder_build_excluded"] is True
    assert result["invariants"]["required_worker_task_kind_execution_coverage_preserved"] is True


def test_v02_125_preserves_architecture_and_full_sidecar_boundary() -> None:
    result = select_slice()
    candidate_ids = [item["slice_id"] for item in result["remaining_candidates"]]
