...
from scripts.v02_119_e08_remaining_sidecar_slice_reselection import COMPLETED_SLICES, select_slice


def test_v02_119_selects_draft_patch_preview_worker_offload_handler() -> None:
    result = select_slice()

    assert result["decision"] == "select_draft_patch_preview_worker_offload_handler"
    assert result["selected_slice"]["slice_id"] == "draft_patch_preview_worker_offload_handler"
    assert result["next_version"] == "v0.2.120_e08_draft_patch_preview_worker_offload_handler"
    assert result["first_design"] == "docs/current-design/design_v0_2_120_e08_draft_patch_preview_worker_offload_handler.md"


def test_v02_119_excludes_completed_test_suite_and_prior_slices() -> None:
    result = select_slice()
...
def test_v02_119_preserves_full_sidecar_boundary() -> None:
    result = select_slice()
    ...
    assert candidate_ids[0] == "draft_patch_preview_worker_offload_handler"
