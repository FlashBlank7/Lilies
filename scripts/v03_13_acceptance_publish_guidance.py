#!/usr/bin/env python3
"""Verify v0.3.13 acceptance/publish guidance and no-build cleanup."""
...
DEFAULT_OUTPUT = ROOT / ".tmp" / "historical-evidence" / "v0.3.13" / "acceptance_publish_guidance_v0.3.13.json"
SMOKE_MARKER = "v0.3.13-smoke"
EXPECTED_RUNTIME_VERSION = "v0.3.6"


BUG_LEDGER = (
    {
        "id": "P1-acceptance-readiness-too-dense",
        "severity": "P1",
        "status": "fixed",
        "reproduction": "The Test tab made users read detailed cards before understanding publish readiness.",
        "fix": "Add a pre-publish acceptance readiness panel.",
        "verification": "acceptance_publish_source_markers.",
    },
    {
        "id": "P1-publish-blocked-without-next-action",
        "severity": "P1",
        "status": "fixed",
        "reproduction": "Publish could be disabled without clear next action.",
        "fix": "Add blocked/ready/published guidance copy.",
        "verification": "acceptance_publish_i18n_markers.",
    },
)
...
def request_json(method: str, url: str, *, token: str = "", payload: dict[str, object] | None = None) -> dict[str, object]:
    ...
    headers = {"Content-Type": "application/json", "User-Agent": "Lilies-v0.3.13-acceptance-publish"}
    if token:
...
def source_marker_checks() -> list[dict[str, object]]:
    checks = [
        (
            "acceptance_publish_source_markers",
            "platform/frontend/app/applications/[id]/page.tsx",
            (
                "acceptanceReadinessItems",
                "publishGuidance",
                'data-acceptance-guidance="readiness-summary"',
                'data-acceptance-guidance="publish-next-action"',
            ),
        ),
        (
            "acceptance_publish_i18n_markers",
            "platform/frontend/lib/i18n.ts",
            (
                "acceptanceReadinessTitle",
                "publishGuidanceBlocked",
                "publishGuidanceReady",
                "publishGuidancePublished",
            ),
        ),
        (
            "acceptance_publish_style_markers",
            "platform/frontend/app/globals.css",
            (
                ".acceptance-readiness-panel",
                ".acceptance-readiness-list",
                ".publish-guidance",
            ),
        ),
    ]
...
def bug_ledger_evidence() -> dict[str, object]:
    ...
    return {"id": "p0_p1_bug_ledger_acceptance_publish", "passed": not blocking, "bug_count": len(BUG_LEDGER), "blocking_bug_count": len(blocking), "bugs": list(BUG_LEDGER)}


def create_payload(now: int | None = None) -> dict[str, str]:
    suffix = now if now is not None else int(time.time())
    requirement = f"[{SMOKE_MARKER}] Acceptance publish guidance smoke app. Create and clean without starting a build."
    return {"name": f"{SMOKE_MARKER} acceptance publish {suffix}", "description": requirement, "requirement": requirement, "mode": "workflow"}
...
def live_checks(api_url: str, token: str) -> tuple[list[dict[str, object]], dict[str, object]]:
    ...
    try:
        ...
        checks.append({"id": "created_acceptance_publish_smoke_app", "passed": created["status_code"] == 201 and bool(application_id), "status_code": created["status_code"], "application_id": application_id})
    except (urllib.error.URLError, TimeoutError, OSError, json.JSONDecodeError) as error:
        checks.append(failed_check("created_acceptance_publish_smoke_app", api_base + "/api/v1/applications", error))
    if application_id:
...
def build_evidence(*, live: bool = False, api_url: str = "http://127.0.0.1:8001", token: str = "") -> dict[str, object]:
    ...
    return {"version": "v0.3.13", "stage": "acceptance_publish_guidance", "status": "passed" if not failed else "failed", "live_checks_enabled": live, "token_source": token_source if live else "not_used", "smoke_marker": SMOKE_MARKER, "bug_ledger": list(BUG_LEDGER), "safety": safety, "checks": checks, "summary": {"failed_check_count": len(failed), "open_p0_p1_bug_count": bug_ledger_evidence()["blocking_bug_count"], "build_endpoint_called": safety["build_endpoint_called"]}}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run v0.3.13 acceptance/publish guidance evidence.")
