#!/usr/bin/env python3
"""Generate v0.2.125 E08 remaining sidecar architecture reselection evidence."""
...
OUTPUT_NAME = "decision_v0.2.125_e08_remaining_sidecar_architecture_reselection"
...
def candidates() -> list[E08RemainingArchitectureSlice]:
    return [
        E08RemainingArchitectureSlice(
            ...
            sidecar_criticality=38,
            readiness=26,
            product_value=34,
            testability=22,
            scope_risk=28,
            existing_evidence="docs/stage-report-archives/v0.2.x/v0.2.112_e08_distributed_heartbeat_registry.md; docs/stage-report-archives/v0.2.x/v0.2.124_e08_builder_build_worker_offload_handler.md",
            next_version="v0.2.126_e08_production_worker_supervision",
            first_design="docs/current-design/design_v0_2_126_e08_production_worker_supervision.md",
        ),
        E08RemainingArchitectureSlice(
            ...
            testability=17,
            scope_risk=35,
            existing_evidence="docs/stage-report-archives/v0.2.x/v0.2.112_e08_distributed_heartbeat_registry.md; platform/backend/src/agent_platform/harness.py",
            next_version="v0.2.126_e08_distributed_queue_semantics",
            first_design="docs/current-design/design_v0_2_126_e08_distributed_queue_semantics.md",
        ),
        E08RemainingArchitectureSlice(
            slice_id="external_kms_provider_integration",
            label="External KMS provider integration",
            sidecar_criticality=27,
            readiness=14,
            product_value=23,
            testability=16,
            scope_risk=31,
            existing_evidence="docs/stage-report-archives/v0.2.x/v0.2.108_e08_secret_kms_rotation_contract.md; platform/backend/src/agent_platform/harness.py",
            next_version="v0.2.126_e08_external_kms_provider_integration",
            first_design="docs/current-design/design_v0_2_126_e08_external_kms_provider_integration.md",
        ),
    ]
...
def select_slice(items: list[E08RemainingArchitectureSlice] | None = None) -> dict[str, Any]:
    ...
    return {
        "version": "v0.2.125",
        "decision_id": "e08_remaining_sidecar_architecture_reselection_after_worker_execution_coverage",
        "source_stage_report": "docs/stage-report-archives/v0.2.x/v0.2.124_e08_builder_build_worker_offload_handler.md",
        "status": "completed",
        ...
        "invariants": {
            "completed_builder_build_excluded": "builder_build_worker_offload_handler" in completed_ids
            and selected.slice_id != "builder_build_worker_offload_handler",
        ...
        "reason": (
            "v0.2.124 closed required worker task-kind execution coverage by implementing builder_build. "
            "The next highest-value remaining architecture slice is production worker supervision because "
            "worker handlers and heartbeat evidence now exist, while distributed queue semantics and external "
            "KMS provider integration remain broader or less ready follow-up slices."
        ),
    }
...
def write_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> tuple[Path, Path]:
    ...
    lines = [
        ...
        f"- Completed builder_build excluded: `{result['invariants']['completed_builder_build_excluded']}`",
