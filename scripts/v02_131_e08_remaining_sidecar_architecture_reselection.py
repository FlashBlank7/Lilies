#!/usr/bin/env python3
"""Generate v0.2.131 E08 remaining sidecar architecture reselection evidence."""
...
OUTPUT_NAME = "decision_v0.2.131_e08_remaining_sidecar_architecture_reselection"


COMPLETED_SLICES = [
    {
        "slice_id": "external_process_manager",
        "evidence": "docs/workingon-archives/v0.2.130/evidence_v0.2.130_e08_external_process_manager_summary.md",
    },
    {
...
def candidates() -> list[E08RemainingArchitectureSlice]:
    return [
        E08RemainingArchitectureSlice(
            ...
            first_design="docs/current-design/design_v0_2_132_e08_external_kms_provider_integration.md",
        )
    ]
...
def select_slice(items: list[E08RemainingArchitectureSlice] | None = None) -> dict[str, Any]:
    ...
    if not open_items:
        raise ValueError("no remaining E08 architecture slice candidates")
    ...
    return {
        ...
        "invariants": {
            "completed_external_process_manager_excluded": "external_process_manager" in completed_ids
            and selected.slice_id != "external_process_manager",
            ...
            "external_kms_provider_integration_selected": selected.slice_id == "external_kms_provider_integration",
        ...
        "reason": (
            "v0.2.130 closed local external process lifecycle management, and the completed architecture set now "
            "covers worker task execution, heartbeat, supervision, distributed queue semantics, and process "
            "management. The remaining concrete sidecar architecture gap is provider-backed external KMS "
            "integration. This selection stage hands that implementation to v0.2.132 while preserving the full "
            "sidecar completion boundary."
        ),
    }
...
def write_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> tuple[Path, Path]:
    ...
    lines = [
        "# v0.2.131 E08 remaining sidecar architecture reselection",
        "",
        ...
        f"- Completed external process manager excluded: `{result['invariants']['completed_external_process_manager_excluded']}`",
        ...
        f"- External KMS provider integration selected: `{result['invariants']['external_kms_provider_integration_selected']}`",
        f"- External KMS provider integration implemented: `{result['invariants']['external_kms_provider_integration_implemented']}`",
