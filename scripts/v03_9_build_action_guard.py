#!/usr/bin/env python3
"""Verify v0.3.9 build actions are explicit, guarded, and no-build by default."""
...
DEFAULT_OUTPUT = ROOT / ".tmp" / "historical-evidence" / "v0.3.9" / "build_action_guard_v0.3.9.json"
SMOKE_MARKER = "v0.3.9-smoke"
EXPECTED_RUNTIME_VERSION = "v0.3.6"


BUG_LEDGER = (
    {
        "id": "P1-home-build-start-unguarded",
        "severity": "P1",
        "status": "fixed",
        "reproduction": "The home primary CTA could start the model team from a first exploratory click.",
        "fix": "Add a first-click confirmation state and keep save-draft as the explicit low-risk path.",
        "verification": "home_build_intent_markers and rendered_home_build_guard.",
    },
    {
        "id": "P1-detail-build-start-unguarded",
        "severity": "P1",
        "status": "fixed",
        "reproduction": "The detail Start Team button did not distinguish inspection from paid model work.",
        "fix": "Add a visible detail-page build intent guard and confirm label before starting a build.",
        "verification": "detail_build_intent_markers and rendered_detail_build_guard.",
    },
    {
        "id": "P1-v039-smoke-retention",
        "severity": "P1",
        "status": "fixed",
        "reproduction": "A live guard harness creates local smoke applications.",
        "fix": "Use the smoke cleanup boundary and verify deletion after the no-build check.",
...
def request_json(
    ...
    *,
    ...
    headers = {"Content-Type": "application/json", "User-Agent": "Lilies-v0.3.9-build-guard"}
    if token:
...
def fetch_text(url: str) -> dict[str, object]:
    request = urllib.request.Request(url, headers={"User-Agent": "Lilies-v0.3.9-build-guard"})
    with urllib.request.urlopen(request, timeout=10) as response:
        body = response.read().decode("utf-8", errors="replace")
...
def source_marker_checks() -> list[dict[str, object]]:
    checks = [
        (
            "home_build_intent_markers",
            "platform/frontend/app/page.tsx",
            (
                "buildIntentConfirmed",
                'data-build-action="home-start-builder-team"',
                "buildIntentHomeConfirm",
                "saveDraftOnly",
            ),
        ),
        (
            "detail_build_intent_markers",
        ...
        (
            "build_intent_copy_markers",
            "platform/frontend/lib/i18n.ts",
            (
                "buildIntentHomeConfirm",
                "buildIntentGuardSafe",
                "createConfirmButton",
                "startTeamConfirm",
            ),
        ),
        (
            "build_intent_style_markers",
...
def bug_ledger_evidence() -> dict[str, object]:
    ...
    return {
        "id": "p0_p1_bug_ledger_build_action_guard",
        "passed": not blocking,
...
def create_payload(now: int | None = None) -> dict[str, str]:
    ...
    requirement = (
        f"[{SMOKE_MARKER}] Build intent guard smoke app. "
        "Create, open, and clean this app without starting the builder team."
    )
    return {
        "name": f"{SMOKE_MARKER} build guard {suffix}",
...
def live_checks(api_url: str, frontend_url: str, token: str) -> tuple[list[dict[str, object]], dict[str, object]]:
    ...
    try:
        ...
        checks.append(
            {
                "id": "rendered_home_build_guard",
                "url": frontend_base + "/",
                "passed": home["status_code"] == 200
                and 'data-build-action="home-start-builder-team"' in home_body
                and 'data-build-intent="needs-confirmation"' in home_body,
                "status_code": home["status_code"],
            }
        )
    except (urllib.error.URLError, TimeoutError, OSError) as error:
        checks.append(failed_check("rendered_home_build_guard", frontend_base + "/", error))
    ...
    try:
        ...
        checks.append(
            {
                "id": "created_build_guard_smoke_app",
        ...
        checks.append(failed_check("created_build_guard_smoke_app", api_base + "/api/v1/applications", error))
    ...
    if application_id:
        ...
        try:
            ...
            checks.append(failed_check("rendered_detail_build_guard", detail_url, error))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run v0.3.9 build action guard evidence.")
