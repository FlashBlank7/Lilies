#!/usr/bin/env python3
"""Verify v0.3.24 requirement readiness summary."""
...
BUG_LEDGER = (
    {
        "id": "P1-create-requirement-readiness-invisible",
        "severity": "P1",
        "status": "fixed",
        "reproduction": "Users could edit a requirement but had no local signal about whether it was specific enough before draft/build.",
        "fix": "Add deterministic requirement readiness summary.",
        "verification": "requirement_readiness_fixture.",
    },
    {
        "id": "P1-create-missing-detail-hints-absent",
        "severity": "P1",
        "status": "fixed",
        "reproduction": "Users needed concrete hints for missing audience, outcome, acceptance, or detail signals.",
        "fix": "Add one visible readiness item per signal.",
        "verification": "requirement_missing_detail_fixture.",
    },
    {
        "id": "P1-v0324-tests-must-enter-release-gate",
        "severity": "P1",
        "status": "fixed",
...
def requirement_readiness(requirement: str) -> dict[str, Any]:
...
def requirement_readiness_fixture() -> dict[str, Any]:
    strong = requirement_readiness("Build a workflow for a customer success operator that classifies customer requests, generates a handling summary, and acceptance must cover billing, delivery, and complaint cases.")
    weak = requirement_readiness("Build a helper.")
    return {
        "id": "requirement_readiness_fixture",
...
def requirement_missing_detail_fixture() -> dict[str, Any]:
    partial = requirement_readiness("For an operator, classify alerts and generate a summary.")
    missing = [signal["id"] for signal in partial["signals"] if not signal["ready"]]
    return {
        "id": "requirement_missing_detail_fixture",
        "passed": partial["ready"] is False and set(missing) == {"acceptance", "detail"},
        "partial": partial,
        "missing": missing,
    }
...
def bug_ledger_evidence() -> dict[str, Any]:
    ...
    return {"id": "p0_p1_bug_ledger_requirement_readiness", "passed": not blocking, "bug_count": len(BUG_LEDGER), "blocking_bug_count": len(blocking), "bugs": list(BUG_LEDGER)}


def request_json(url: str) -> dict[str, Any]:
    request = urllib.request.Request(url, method="GET", headers={"User-Agent": "Lilies-v0.3.24-requirement-readiness"})
...
def build_evidence(*, live: bool = False, api_url: str = "http://127.0.0.1:8001") -> dict[str, Any]:
    checks: list[dict[str, Any]] = [bug_ledger_evidence(), requirement_readiness_fixture(), requirement_missing_detail_fixture(), *source_marker_checks()]
    ...
    checks.append({"id": "safety_no_forbidden_requirement_readiness_call", "passed": safety["forbidden_endpoint_called"] is False and safety["model_call_used"] is False, "called_endpoints": safety["called_endpoints"], "forbidden_endpoint_fragments": list(FORBIDDEN_ENDPOINTS), "model_call_used": safety["model_call_used"]})
    failed = [check for check in checks if not check.get("passed")]
    return {
        "version": "v0.3.24",
        "stage": "requirement_readiness_summary",
        "status": "passed" if not failed else "failed",
        "live_checks_enabled": live,
        "safety": safety,
        "bug_ledger": list(BUG_LEDGER),
        "checks": checks,
        "summary": {"failed_check_count": len(failed), "open_p0_p1_bug_count": bug_ledger_evidence()["blocking_bug_count"], "forbidden_endpoint_called": safety["forbidden_endpoint_called"], "readiness": requirement_readiness_fixture(), "missing_detail": requirement_missing_detail_fixture()},
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run v0.3.24 requirement readiness evidence.")
