...
it('creates an editable retrieval workflow before a connection or index exists', async () => {
  ...
  fireEvent.click(screen.getByRole('button', {name:'创建检索工作流'}))
  ...
  expect(JSON.parse(save[1]!.body as string)).toMatchObject({expected_revision:0, workflow:{nodes:[{type:'start'}, {type:'knowledge_search',config:{knowledge_ref:'manual'}}, {type:'end'}]}})
