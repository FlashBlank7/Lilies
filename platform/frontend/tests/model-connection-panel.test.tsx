...
it('configures an API model, thinking and workflow access without persisting key in the form', async () => {
  ...
  await waitFor(() => expect(saved).toHaveBeenCalledOnce())
  expect(JSON.parse(mockApi.mock.calls[0][1]!.body as string)).toMatchObject({ provider: 'api', api_key: 'private-key', model: 'review-model', thinking: 'high', runtime_enabled: true })
...
it('supports Anthropic thinking switches and prevents changes while a turn runs', () => {
