...
it('separates generation configuration and supports explicit inheritance without sending main credentials', async () => {
  mockApi.mockResolvedValue({ mode: 'inherit', provider: 'api', model: 'business-model', has_api_key: true, runtime_enabled: true })
  const saved = vi.fn().mockResolvedValue(undefined)
  render(<ModelConnectionPanel base="/api/v1/projects/p" role="generation" connected running={false} onSaved={saved} />)
  fireEvent.click(screen.getByRole('button', { name: '工作流生成模型设置' }))
  await screen.findByLabelText('生成模型来源')
  expect(screen.getByText(/沿用主模型：business-model/)).toBeInTheDocument()
  expect(screen.queryByLabelText('API Key')).not.toBeInTheDocument()
  fireEvent.change(screen.getByLabelText('生成模型来源'), { target: { value: 'independent' } })
  expect(screen.getByLabelText('模型')).toHaveValue('')
  expect(screen.getByLabelText('API Key')).toHaveAttribute('placeholder', '填写服务商提供的 API Key')
  fireEvent.change(screen.getByLabelText('API 地址'), { target: { value: 'https://generator.test/v1' } })
  fireEvent.change(screen.getByLabelText('API Key'), { target: { value: 'generation-key' } })
  fireEvent.change(screen.getByLabelText('模型'), { target: { value: 'generator-model' } })
  fireEvent.click(screen.getByRole('button', { name: '保存模型连接' }))
  await waitFor(() => expect(saved).toHaveBeenCalledOnce())
  expect(mockApi.mock.calls[1][0]).toBe('/api/v1/projects/p/generation-model')
  expect(JSON.parse(mockApi.mock.calls[1][1]!.body as string)).toMatchObject({ model: 'generator-model', api_key: 'generation-key' })
  fireEvent.click(screen.getByRole('button', { name: '工作流生成模型设置' }))
  await screen.findByLabelText('生成模型来源')
  fireEvent.click(screen.getByRole('button', { name: '保存模型连接' }))
