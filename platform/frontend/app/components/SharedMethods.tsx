...
export default function SharedMethods({projectId,onChanged}:{projectId:string;onChanged:()=>unknown}) {
  ...
  return <section className={styles.section}><h2>内部共享方法与流程</h2><p>主动分享给指定项目；加入后得到独立可编辑副本，后续分享不会覆盖它。</p><button disabled={busy} onClick={()=>void begin()}>分享本项目的方法或流程</button>
    ...
    {open&&<ReadingDialog title="分享方法或工作流" onClose={()=>setOpen(false)}><div className={styles.section}>
      ...
      <label>要分享的内容<select value={selected} onChange={e=>{setSelected(e.target.value);setPreview('');setError('');setSkill(null);setReferenceNames([]);const [kind,...ids]=e.target.value.split(':');setName((kind==='workflow'?workflows:skills).find(x=>x.id===ids.join(':'))?.name||'')}}><option value="">请选择</option><optgroup label="工作流">{workflows.map(x=><option key={x.id} value={'workflow:'+x.id}>{x.name}</option>)}</optgroup><optgroup label="方法说明">{skills.map(x=><option key={x.id} value={'skill:'+x.id}>{x.name}</option>)}</optgroup></select></label>
      {selectedSkill&&<fieldset><legend>一起分享的引用文件</legend>{!referencesReady?<p role="status">正在读取引用文件…</p>:!Object.keys(skill?.references||{}).length?<p>这篇方法没有引用文件。</p>:<><p>只复制勾选文件的本次版本。接收方可独立修改；未勾选的资料不会带入。</p>{Object.entries(skill?.references||{}).map(([file,content])=><label key={file} style={{overflowWrap:'anywhere'}}><input type="checkbox" checked={referenceNames.includes(file)} onChange={e=>{setReferenceNames(e.target.checked?[...referenceNames,file]:referenceNames.filter(x=>x!==file));setPreview('')}}/>{file}（{content.length.toLocaleString()} 字符）</label>)}</>}</fieldset>}
