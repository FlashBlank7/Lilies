      ...
      {loading && !overview ? <div className={styles.loadingState}><LoaderCircle className={styles.spin} size={24} /><strong>Loading platform evidence</strong></div> : <div className={styles.content}>
        ...
        {tab === 'Cost & Tokens' && <>
          ...
          <section className={styles.splitSections}>
            ...
            <div className={styles.dataSection}><header><div><h2>By stage</h2><p>Builder, workflow-node, intake, repair, and collaboration phases.</p></div></header><div className={styles.compactList}>{(usage?.dimensions.phase || []).map(item => <div key={String(item.phase)}><span><strong>{String(item.phase)}</strong><small>{number(Number(item.calls))} calls</small></span><b>{number(Number(item.tokens))}</b><em>{money(Number(item.cost_usd))}</em></div>)}</div></div>
