...
export default function Home() {
  ...
  return (
    <main className="home-shell">
      <nav className="topbar"><div className="brand"><span>F</span> Foundry</div><div className="topbar-actions"><Link href="/projects">打开项目</Link><button className="lang-toggle" onClick={toggleLocale}>{t.switchLabel}</button><div className={`status-dot runtime-status ${runtimeStatus}`} data-runtime-status={runtimeStatus}><span>{runtimeStatusText}</span><small>{runtimeStatusDetail}</small></div></div></nav>
      <section className="hero">
        <h1>{locale === 'zh' ? '历史工作流与开发工具' : 'Legacy workflows and developer tools'}</h1>
        <p>{locale === 'zh' ? '新需求从项目开始，需求沟通、工作流与结果都保存在同一个项目中。' : 'Start new work in a project to keep the conversation, workflows and results together.'} <Link href="/projects">{locale === 'zh' ? '打开项目' : 'Open a project'}</Link></p>
        <details><summary>{locale === 'zh' ? '旧版搭建工具' : 'Legacy builder tools'}</summary>
        <RequirementPackageImport locale={locale} disabled={busy || draftBusy}
          onImported={id => { window.location.href = `/projects/${id}` }} />
