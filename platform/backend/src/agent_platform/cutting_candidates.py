...
不同物料的候选都分别使用同一份剩余需求上限，不能把各根首选直接合并。可将比较后的候选、patterns.csv和原需求表交给“候选按共享数量分配”逐次扣减共同需求；设备、配对和时间仍需领域逻辑。这里不证明全局最优、不消耗库存、不回写现场。改变输入或参数新开运行，旧报告保留；改比较偏好只调用已有比较流程，不需重新生成。'''
