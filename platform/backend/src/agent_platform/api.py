...
def create_app(settings: Settings | None = None, provider: ModelProvider | None = None) -> FastAPI:
    ...
    app.state.workflow_runtime = services.workflow_runtime
    ...
    # ── Block Family Strategies ──────────────────────────────

    @app.get(
        "/api/v1/block-families/strategies",
        dependencies=[Depends(require_token)],
    )
    async def list_block_strategies(
        family: str | None = None,
    ) -> dict[str, Any]:
        """List available block strategies grouped by family (context/model/tool/governance/agent/skill)."""
