...
- cases: 1 到 8 条用例，每条：
  - name: 业务化名字
  - inputs: 完整可运行的输入对象（字段名必须严格用系统对接说明里的；业主例子里给了数据
    就一个字都不改地原样用，没给的才合理补全）
  ...
  - expect:
    ...
    - equals: 输出字段必须相等的值（只写业主明确说了"该出什么"的；布尔/数字/短文本）
      【期望值与输入是绑定的一对】业主说的"该出什么"永远是针对他那个输入说的。
      你若换了输入、改了数据、补全了字段，业主的数字立刻失效——要么把输入原样搬回来，
      要么自己按新输入重新算一遍期望值。绝不允许"新输入 + 业主旧数字"这种组合。
      自己算不出来的，就别写进 equals（写进 suggestions 让业主自己定）。
...
- 【期望跟着输入走】改了输入就必须重算期望值。判错一个好工作流比漏验更贵：
  业主会被你引去修一个根本没坏的东西。拿不准时宁可少写一条 equals。
- 用例输入必须自洽完整，能直接跑。
- 绝不发明对接说明之外的字段名。
...
- 期望跟着输入走：业主的"该出什么"只对他那个输入成立。换了输入就得自己重算期望值，
  绝不能拿新输入配业主的旧数字——判错一个好工作流会把业主引去修没坏的东西。
"""
