...
2026-09-17 方向修正：项目主路径仅连接模型 API，由平台管理智能体循环、工具调用、会话状态与停止恢复。思考模式可配置，显式启用后同一模型用于成员工作流。之前用本机 Codex 完成的试用只证明外部 Agent 接入，不能证明平台目标达成；上汽项目需从原始资料重新试用。详见[项目模型连接](project-model-connections.md)。
...
客户从“打开项目”开始或继续所有工作，简单需求也以项目组织。平台首页进入项目列表；“打开新项目”和“导入并打开项目”只建立项目及保存资料，用户发出指令后才分析和搭建。一次试用或业务执行记录留在项目内，后续沟通继续原项目。历史独立工作流与开发工具在 `/applications` 保留兼容入口，不自动迁移。

项目协作增加 Project 容器：一个项目会话、一条主工作流和多个成员 Application，共享业务记录并通过主流程拓扑协作。成员仍各自保留草稿、测试与版本。项目任务固定所有成员草稿，关联主运行、成员运行和业务结果。具体接口、恢复规则和测试入口见 [项目内多工作流协作](project-cooperation.md)。现有单应用方式继续保留。

项目首页进一步以业务进展和统一对话组织客户使用：可用程度与建设状态分开，客户通过对话试用、反馈和继续，平台保持事项与实际任务关联。详见 [项目进展与统一对话](project-conversation.md)。

项目内建模增加数据集、建模研究和实验候选：统筹分析需求、提出特征或模型改动，实际运行器执行标准库训练、计算指标并保存最佳模型。客户在同一对话查看数据、特征和实验，反馈后继续，最终复用预测工作流。详见 [数据分析与自主建模](modeling.md)。

### 3.1 Application

项目中的工作流载体，继续承载草稿、测试和版本。一个 Application 可以是：
