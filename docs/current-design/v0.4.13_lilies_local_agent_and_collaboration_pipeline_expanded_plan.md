...
| 计划版本 | Draft 3 |
| 对应阶段任务 | `V04-13-T01` |
| 来源意图 | `PRODUCT-001`、`PRODUCT-005`、`PRODUCT-008`、`PRODUCT-015`、`PRODUCT-016`、`PRODUCT-017`、`ARCH-003`、`EVAL-003`、`GOV-002` |
...
| 实施限制 | r6 按最新用户优先级执行 T01K→L→M→N，再恢复 T01H→I→J；产品目标、角色、审批权、黑箱边界、客户分母和正式题目变化必须重新审批 |
...
| `LILIES-I022` | 解耦后的莉莉丝必须位于当前平台同级的 `../LiliesAgent/` 独立仓库，拥有自己的发行、daemon/CLI、私有状态、测试、Git 历史、原生软件界面和 Token 账本；平台只能安全发现并显式配对，发现不等于授权 | 继续把平台内模块包装成“独立项目”，共享 SQLite/token，读取 bootstrap secret，或发现后自动授权 |
| `LILIES-I023` | 解耦完成后，Android 与 Windows 必须作为两个独立软件开发项目由莉莉丝在冻结 DevelopmentAssignment 内完成；每个项目保留自己的仓库、迭代、预算、Token、构建和真实运行验收，且固定不进入企业工作流分母 | Codex 直接代写、只生成空工程、把 macOS 编译当 Windows 运行，或用一个项目结果替代另一个 |
...
六个真实项目暴露给 Builder 的公开输入统一采用“客户需求包”形式，而不是只给
一段人工整理到完备状态的题面。需求包模拟客户在实施开始时实际能够提供的材料，
内容可以稀疏、不一致或缺少非关键细节；莉莉丝必须识别歧义和必要缺口，不能用
隐藏答案或猜测补齐。每个冻结 revision 至少提供一个公开 manifest，并按实际可用
情况引用以下材料：

- 客户原始需求、目标角色、当前流程和期望交付物；
- 客户交付的 PDF、图片、表格、样例记录、历史输出或其它原始材料；
- 现有系统的公开文档、OpenAPI、SDK/CLI 说明、数据字典或导出样例；
- 已知业务规则、权限/网络边界、secret ref 和允许的测试环境；
- 客户确实提供过的输出模板、公开验收示例和澄清回复。

并非每个需求包都必须包含上述每类材料；缺失项在 manifest 中如实标为未提供。
隐藏 seed 和 oracle 只改变验收样本，不能隐瞒完成任务所必需且真实客户本应提供的
系统接入信息或业务决定。需要客户澄清的内容通过公开、版本化的需求补充进入后续
revision，并计入题包责任和运行记录。

项目 1 的后续 revision 将把现有 requirement、24 条公开业务材料、官方接口材料、
Builder API manual 和输出说明投影成这一需求包视图，不改变业务题、隐藏 seed 或
oracle。项目 2–6 在各自第一次 Builder attempt 前必须先冻结同样的需求包 manifest。

积木手册的渐进披露和工作流搭建 Skill 属于后续产品能力，本轮不提前实施。当前可
继续使用冻结的完整积木/API 手册；未来 Skill 必须描述通用的需求理解、选积木、
搭建和验证方法，不能包含项目专用映射、最终图或隐藏验收答案。
...
| 依赖 | T01G |
| 具体改动 | 锁定六项目 registry 和各项目 manifest；项目 1 保留 revision 1–20 及全部历史 Lilies 失败，项目 2–6 各自从 revision 1 开始；严格逐个真实启动。六项目公开输入均以第 17.1.1 节的客户需求包形式提供，允许材料像真实客户输入一样不完备，但不得隐藏完成任务所必需的接入事实。每个项目由未读平台源码和隐藏证据的全新隔离 Codex 黑箱 Builder 开始，并在该项目的修复/复测中复用同一持久 thread；Provider 默认断路且每次真实调用需要显式放行，runner 持久记录进程、阶段、Token、调用数、成本及缺失账本状态。 |
| 动态缺口 ID | 每个获批通用报告在独立 ledger 生成 `CAP-LILIES-<short-id>`，验收来自报告的 missing_contract，不进入任何客户分母 |
| Codex 边界 | Builder Codex 只能看到客户需求包、过滤后的 task workspace、冻结公开手册和 task-scoped 平台接口，并只能经这些接口创建、修改、测试、运行、检查、交付工作流；不得读取仓库源码、Git、平台数据库、developer workspace、protected/expected/oracle。当前 developer Codex 与 Builder 上下文严格分离，只能实现通用平台能力并回传验证，不能代写项目工作流或项目专用 adapter、mapping、wrapper。 |
| 禁止捷径 | 不根据当前 developer 已知的积木缺口预写最终图；必须先由隔离 Builder 通过公开路线实测并报告缺口，再按本题 `auto_forward` 无人监管策略路由。每次补能必须保存 Builder gap report → developer 通用补能与验证 → 同一 Builder thread 的受影响项目 rerun/reprobe 三联因果历史；auto 不得绕过网络、秘密、预算、宿主副作用、oracle 或其它冻结授权。 |
| 验收 | 项目 1 使用第 18.6 节 oracle；项目 2–6 使用各 manifest 冻结后的独立业务 oracle；六项目必须分别通过，若预算内未通过则如实 fail。每项目单独报告完成情况、已补平台能力、未补软件/模型供应缺口、硬件/性能/网络等环境限制、证据等级、声明上限和复查触发条件。组合必须覆盖 ML 训练、ML 模型调用、RAG 检索和嵌入特定系统；EXP-LILIES-006 必须完成时间切分、训练/拟合、评估/回测、不可变模型版本、部署/提升、推理和重训触发的完整生命周期。 |
| 证据 | 组合与项目 manifest、每项目完整题包/revision chain、actor-stamped Codex thread、过滤 workspace manifest、task credential、完整 public API transcript、reports/runs/oracle/指标包和 forbidden-assistance scan；以及每次独立 CAP-LILIES 的 Builder report、developer diff/tests/review、同项目 rerun/reprobe 和可重放 Collaboration 三联历史。 |
...
| 关闭 | 同一隔离 Codex Builder thread 经公开接口复验、三联 Collaboration 历史完整且最终独立 oracle 通过 |
...
| 字段 | 内容 |
| --- | --- |
| 依赖 | T01A-I 与 T01K-N 全部达到各自声明证据 |
| 具体改动 | 中文阶段报告、覆盖 A-I/K-N 的机器 release gate、claim ceiling、design/workingon/evidence 归档 |
...
| 证据 | closure review、release gate JSON、Q01-Q28/协同开发/企业实验/独立仓库/Android/Windows archive digest、所有仓库提交 SHA |

### 20.11 `V04-13-T01K`：同级独立 LiliesAgent 与原生软件界面

| 字段 | 内容 |
| --- | --- |
| 来源意图 | `LILIES-I022`、`PRODUCT-001`、`PRODUCT-017`、`ARCH-003`、`EVAL-003`、`GOV-002` |
| 依赖 | T01A、T01C、T01G 的已验证协议边界；不依赖 T01H 客户结果 |
| 固定位置 | 当前平台仓库同级 `../LiliesAgent/`，即 `agent/` 目录下的独立 Git 仓库 |
| 具体改动 | 自有 Python distribution、daemon/CLI、状态目录、测试、Git 历史和 macOS SwiftPM 原生应用；平台只保留 HTTP bridge/client 与安全 discovery adapter |
| 发现与配对 | discovery record 必须同一用户、0600、非符号链接且只含公开候选信息；平台核对 loopback health 指纹。一次性配对必须由用户从独立软件生成；平台不得读取 bootstrap secret，发现不授予任何 scope |
| Token 边界 | Provider 默认失败关闭；启动和空闲不得触发模型。usage 按 session/stage/model 持久聚合，缺失调用标记 unknown，不估算；界面显示总量和分阶段/模型明细 |
| 禁止捷径 | 不 import `agent_platform`，不共享平台 SQLite、token 或私密 key；不把平台内旧模块、macOS build 成功或 discovery 候选写成整个任务完成 |
| 验收 | package/Python/Swift 测试、独立进程桥和重启、权限/owner/no-follow/fingerprint/bootstrap/pairing 反例、真实应用启动与视觉检查、空闲零 Token 审计、独立安全复审全部满足 r6 锁 |
| 证据 | 只归档脱敏 manifest、命令结果、截图和两个仓库 commit；禁止复制数据库、bootstrap token、bearer 或 Keychain 内容 |

### 20.12 `V04-13-T01L`：独立 DevelopmentAssignment 执行底座

| 字段 | 内容 |
| --- | --- |
| 来源意图 | `LILIES-I020`、`LILIES-I021`、`LILIES-I023`、`PRODUCT-001`、`PRODUCT-017`、`ARCH-003`、`EVAL-003`、`GOV-002` |
| 依赖 | T01K |
| 具体改动 | 在独立仓库内提供 DevelopmentAssignment、grant、work item、result/review、平台中立 API/CLI 和模型—工具循环；角色、路径、精确 argv、网络、副作用、deadline、Token/cost/tool/IO 预算全部冻结 |
| 无人监管边界 | autonomous 只取消逐条人工交接；executor 与 resolver 必须是不同授权主体，executor 不得创建、批准或扩大自己的 grant |
| 副作用耐久性 | 执行前持久写入 prepared；running/prepared 在不确定重启后进入 reconciliation_required，禁止盲目重放；稳定 mutation idempotency key 防止重复副作用 |
| 工具安全 | 文件与进程 I/O 有界、可取消、不阻塞事件循环；workspace 与 agent state 不得重叠；symlink/path/network/command escape 失败关闭；正常退出、超限和取消均清理完整进程组 |
| Token 边界 | 模型调用写入同一 stage/model ledger；Provider 关闭时不可调用，未知 usage 不伪造为 0 |
| 验收 | 全部安全/恢复/幂等负向和独立进程 API/CLI 通过，并由 deterministic provider 完成一个含 diff、测试和 usage 的端到端开发 fixture；这仍不是 Android/Windows 客户结果 |

### 20.13 `V04-13-T01M`：Android 独立软件开发项目

| 字段 | 内容 |
| --- | --- |
| 来源意图 | `LILIES-I023`、`PRODUCT-001`、`PRODUCT-017`、`EVAL-003`、`GOV-002` |
| 依赖 | T01K、T01L |
| 项目冻结 | 执行前建立独立 project manifest，固定应用需求、空 baseline、仓库、允许工具、Android toolchain、预算、验收 oracle 和停止条件 |
| 莉莉丝边界 | 使用新的隔离 session 与 autonomous DevelopmentAssignment；Codex 只可准备环境、检查证据或处理新的明确授权，不能直接实现应用 |
| 验收 | 每次迭代归入该项目；交付源码、测试、可复现 APK/application bundle，并在真实 Android 模拟器或设备完成冻结 smoke/oracle |
| 禁止捷径 | 空工程、静态源码扫描、未运行 APK、其它项目结果或直接 Codex 实现均不算完成 |
| 分母 | `enterprise_denominator=false`，不能改变 T01H 的 `0/3` 或六客户项目分母 |
| 证据 | 题面/baseline digest、assignment/grant、完整 model/token/tool transcript、逐迭代 commit/diff/test、工具链锁、artifact digest、模拟器/设备截图日志、验收与独立 review |

### 20.14 `V04-13-T01N`：Windows 独立软件开发项目

| 字段 | 内容 |
| --- | --- |
| 来源意图 | `LILIES-I023`、`PRODUCT-001`、`PRODUCT-017`、`EVAL-003`、`GOV-002` |
| 依赖 | T01K、T01L；可以在 M 后执行但不得共享上下文、授权或结果 |
| 项目冻结 | 执行前建立独立 project manifest，固定应用需求、空 baseline、仓库、允许工具、Windows toolchain、预算、验收 oracle 和停止条件 |
| 莉莉丝边界 | 使用新的隔离 session 与 autonomous DevelopmentAssignment；Codex 不得直接实现 Windows 应用 |
| 验收 | 每次迭代归入该项目；交付源码、测试和可复现 Windows 应用工件，并在真实 Windows runtime 完成冻结 smoke/oracle |
| 禁止捷径 | 空工程、仅 macOS 编译、兼容层静态结果、Android 结果或直接 Codex 实现均不算完成 |
| 分母 | `enterprise_denominator=false`，不能改变 T01H 的 `0/3` 或六客户项目分母 |
| 证据 | 题面/baseline digest、assignment/grant、完整 model/token/tool transcript、逐迭代 commit/diff/test、工具链锁、artifact digest、真实 Windows 截图日志、验收与独立 review |
...
| 20 | 2026-07-26 | 已批准实现路线偏移 | 冻结 parent revision 18 的 revision 19：题面与 r18 逐字节相同，24 个公开 PDF 逐字节相同，148 个不可变文件、三个各 36 条 hidden seed、118 项 oracle、预算、权限、真实宿主、模型出口、官方 Token 监控、CAP 准入和验收保持不变。公开/密封 package digest 为 `047e…c4fa` / `25fa…9103`，verification process digest 仍为 `fbef…cab`；preparation source revision `27a4…6c44` 现显式绑定 `collaboration_service.py`。唯一代码差异是已验证的通用 report semantic-completeness 响应诊断；冻结后 153 项 package/runner/monitor/collaboration/formal 组合回归、portfolio/evolution validators、Ruff、JSON 与 diff check 通过。下一步从全新状态运行 r19 seed 101。 |
| 21 | 2026-07-26 | 用户要求的可观察运行 | revision 19 public debug observation 以 600 秒 Builder 硬上限启动真实 Paperless/InvenTree 环境和正式平台边界。公开信息流显示莉莉丝依次读取 application/contract/block/tool catalog，然后在端口类型、nested edge、structured LLM output、stale reference 和 Write string content 之间反复修改 Draft。该 run 共 69 次模型调用、9,325,430 tokens、92 次 Builder tool、45 次成功 draft apply、60 次 context update，三个 workflow run 全部失败；宿主请求、connector、artifact、receipt、report 和 claim 均为 0。没有 capability report，因此没有莉莉丝—Codex developer handoff。进程已在 deadline 后停止，证据作为不进入企业分母的 debug failure 保留。 |
| 22 | 2026-07-26 | 用户批准 D2 合同修订 | 用户明确要求由 Codex 替代莉莉丝完成六个项目，只使用平台公开接口搭建工作流；发现平台软件限制时先做通用补能并重跑，实在无法补能则留下缺口；每项目分别报告完成情况、已补平台功能、未补功能，并区分硬件/性能/网络/模型供应限制与平台软件缺失。合同修订为 revision 4：当前已读源码的 developer Codex 不得冒充黑箱 Builder；另起全新隔离 Codex Builder，只得到题面、过滤 workspace 和 task-scoped API，不得读取源码、Git、平台数据库、developer workspace、protected/expected/oracle。历史 Lilies attempt 不清零，六项目、顺序、独立分母/oracle、CAP 禁止宿主专用 adapter/mapping/wrapper/final graph 和重跑要求不变。四类工作流为 ML 训练、ML 模型调用、RAG 与嵌入特定客户系统；EXP-LILIES-006 强制承担不可绕过的完整 ML 训练生命周期主分母。 |
| 23 | 2026-07-26 | 用户直接批准的新产品边界 | 用户要求把进程解耦后的莉莉丝做成当前平台仓库上一级 `agent/` 目录中的同级独立项目，并提供类似 Codex/Claude Code 的软件界面；解耦后再由独立莉莉丝分别承接 Android 与 Windows 软件开发项目。独立仓库固定为 `../LiliesAgent/`，拥有自己的 Python distribution、daemon/CLI、状态目录、测试、macOS SwiftPM 应用和 Git 历史。平台保留 bridge/client，不得 import 独立项目源码或共享 SQLite/Token；只允许读取同一用户的安全 discovery record、核对 loopback daemon 公共指纹，并在用户提供一次性码后显式配对。自动发现不等于自动授权，Provider 出口仍默认关闭。该用户指令暂时优先于原 T01H 顺序，但不得把包装、GUI 或 Android/Windows 工程结构误报为六个企业项目通过。 |
| 24 | 2026-07-26 | 用户批准原 T01H 目标修订与记录门禁 | 用户要求回到原 `V04-13-T01H`，由 Codex 代替莉莉丝仅调用平台接口做题和驱动补能；今后沟通、合作和补能记录必须保存。合同 revision 5 因而保持同一任务 ID、不新增替代任务，并规定每个补能周期只有在 Collaboration 中持久存在 Builder gap report、developer 通用补能与验证、同一 Builder thread 的项目 rerun/reprobe 三联记录后才可计入项目完成。revision 20 进一步把 Builder API manual 纳入不可变 public workspace manifest；历史 r1–r19 与所有 Lilies 失败不改写。 |
| 25 | 2026-07-26 | 用户批准 contract revision 6 并明确最终目录 | 用户再次明确独立莉莉丝项目必须放在当前平台项目上一级，即 `agent/LiliesAgent`。r6 保留 r5 的 A-I/H 锁和 T01H `0/3` 事实，新增 T01K 同级独立发行与界面、T01L DevelopmentAssignment、T01M Android、T01N Windows，并只扩展 T01J 发布门。当前任务改为 K，顺序为 K→L→M→N→恢复 H→I→J；K-N 在真实证据和独立审查前均不得写成完成，且固定 `enterprise_denominator=false`。 |
| 26 | 2026-07-26 | 用户批准六项目客户需求包形态 | 用户明确 Builder 的公开输入不只有需求文字，还包括客户实际提供的文档和完成任务所需材料；真实客户输入可以不详细，莉莉丝后续通过材料、积木手册和通用 Skill 理解并完成需求。六项目因此统一采用客户需求包视图：公开 manifest 引用客户原始需求、实际样例材料、系统接口资料、已知规则、环境与交付说明，缺失项如实保留并由莉莉丝识别，hidden seed/oracle 只用于独立验收，不能隐瞒必要接入事实。积木手册渐进披露和 Skill 编写暂缓，不作为当前项目 1 的前置条件；未来 Skill 不得编码项目专用映射、最终图或隐藏答案。 |
| 27 | 2026-07-26 | 项目一 revision 26 通用合同与启动顺序补能 | revision 25 正式启动在 Builder 交接前暴露通用合同投影缺陷：预分配摘要只携带 scope/application，却在 Connector 授权上线后需要完整冻结动作与宿主策略；同时 Connector 若在交接后注册会改变工具目录 digest。revision 26 保留全部客户输入、宿主操作、隐藏 seed、oracle、预算和 `0/3` 分母，只把冻结 `AllowedActionsPolicy` 贯穿 broker/runtime/contract facade，并新增通用 `owner-setup-only` 两阶段启动，使所有者先通过通用接口完成官方 Connector generation/binding/policy 配置，再生成不可变 Builder 合同。该补能不提供 Paperless/InvenTree 业务映射、最终图或答案；r25 失败启动作为无模型调用的集成证据保留。 |
| 28 | 2026-07-26 | 项目一 revision 27 题方澄清与同线程复测 | r26 新鲜隔离 Builder 已只用公开接口读取需求包、应用和合同，且 Connector task projection 复核通过；它随后把“客户未提供字段映射、最终工作流、workbook 模板”全部解释为必需前置材料，持久提交 `task_spec_gap` 报告 `d546d2ae-a6bb-4bc1-a96f-56bd9e4d3b07` 并路由题方，未创建工作流。用户先前已明确真实客户材料可以不完备，Builder 应通过材料、积木手册与通用能力完成需求。因此 r27 保留全部业务/hidden/oracle/宿主/预算/`0/3` 分母，只澄清通用需求包语义：最终工作流和 workbook 是 Builder 交付物；字段映射先由 Builder 依据客户材料和官方 schema 推导，只有具体必需事实仍歧义且会使治理写入不安全时才报告 gap。不得提供项目专用映射、最终图或答案；r26 report 与同一 Codex thread 必须保留并进入 r27 rerun 证据。 |
...
r6 起物理仓库边界同时冻结：通用莉莉丝 daemon、CLI、状态、provider、DevelopmentAssignment 和桌面应用的最终所有者是 `../LiliesAgent/`；平台仓库只拥有 discovery、HTTP bridge/client 和平台业务适配。平台内既有 `lilies_*` 文件只可作为迁移来源或平台适配，不能继续成为独立发行的运行时依赖。Android 与 Windows 的源码位置由各自冻结 project manifest 指定，必须是两个独立 Git 项目，不能放进平台仓库或用同一迭代链。

| 模块 | 固定职责 | 不得承担 |
| --- | --- | --- |
| `../LiliesAgent/src/lilies_agent/` | 独立 distribution、daemon/CLI、session/state/provider、pairing、usage、DevelopmentAssignment 与受限工具 | import `agent_platform`、读取平台 SQLite/token、包含客户工作流业务 |
| `../LiliesAgent/macos/` | SwiftPM 原生桌面应用、Keychain 客户端状态、会话/消息/权限/Token UI | 保存 bootstrap/bearer 到普通文件、把 discovery 当授权 |
| 平台 `local_lilies_discovery.py`、bridge/client | 安全读取公开候选、核对 loopback 指纹、显式配对和 task-scoped HTTP 适配 | 读取 daemon bootstrap/key/SQLite、import sibling 源码、静默配对 |
| Android / Windows project manifest | 分别冻结实际仓库、题面、baseline、toolchain、grant、预算和 oracle | 合并项目、共享 session/grant、进入企业工作流分母 |
...
通用 AgentRuntime 的既有抽取结果作为迁移输入；r6 后莉莉丝 factory 只由 `../LiliesAgent/` 构造 provider、core tools、sandbox、permission、harness 和独立 storage，平台不得再 import 或调用该 factory，只能经 HTTP 连接 daemon。莉莉丝 factory 不得构造 blocks、WorkflowStorage、WorkflowBuilder、WorkflowRuntime、connectors 或 governance。
...
| `LILIES-I022` | 0、2、20.11、23.3、24 | T01K、T01J | 同级独立仓库、无平台 import/共享状态、安全 discovery/显式 pairing、macOS 应用、空闲零 Token |
| `LILIES-I023` | 2.4、20.12–20.14 | T01L、T01M、T01N、T01J | 冻结 DevelopmentAssignment、两个独立仓库与 transcript、Android 真实设备/模拟器、Windows 真实 runtime、`enterprise_denominator=false` |
| `PRODUCT-017` | 0、2.4、10.4、20.7、20.11–20.14 | T01G、T01H、T01K-N、T01J | manual/autonomous、跨软件 assignment、同级独立软件、受限真实交接和两个不进入企业分母的软件项目 |
