# v0.4.13 莉莉丝本地智能体、黑箱平台接耦与临时合作管道展开实施总计划
...
| 计划版本 | Draft 2 |
| 对应阶段任务 | `V04-13-T01` |
| 来源意图 | `PRODUCT-001`、`PRODUCT-005`、`PRODUCT-008`、`PRODUCT-015`、`PRODUCT-016`、`ARCH-003`、`EVAL-003` |
| 权威来源 | 用户关于莉莉丝独立运行、黑箱搭建、临时合作管道、用户审批、题包责任和独立复验的直接指令 |
| 前置阶段 | `v0.4.12` 完成有限闭环清障并通过独立 Closure Audit |
| 批准依据 | 用户于 2026-07-22 明确批准“按照生成的总计划去做”，四个审批包全部通过 |
...
临时合作管道只服务开发期正式实验。当莉莉丝在预检、规划、搭建、运行或验收的任何时刻发现平台通用能力缺失或疑似实现错误时，它可以提交证据报告。报告先由用户查看；用户批准后才进入 Codex 缓冲区。Codex 修复平台并回传实现证据，莉莉丝重新读取黑箱能力合同，自行判断是否继续。莉莉丝认为完成后仍需 Codex 使用隐藏 oracle 独立复验。

该管道不是莉莉丝的常驻工具，不进入普通系统提示词，不在非实验任务的工具目录中出现。没有激活管道时，莉莉丝必须不知道该能力存在。
...
| `LILIES-I003` | 莉莉丝只能看到公开积木、手册、合同、草稿、运行和验收接口 | 向莉莉丝挂载平台仓库、数据库或内部 Python 对象 |
...
| `LILIES-I007` | 平台通用能力缺失和疑似平台缺陷必须先让用户审批 | Codex 后台自动读取并开始改代码 |
...
| `LILIES-I012` | 权限和高风险动作进入用户授权，不算平台缺功能 | 将用户拒绝授权上报为能力缺口 |
...
普通客户模式是成熟产品形态。客户输入可能不完整，莉莉丝可以通过平台已有的选项式需求补全获得业务角色、输入、输出、权限和验收信息。普通客户模式没有合作管道，也没有 Codex 等待在另一端。
...
| 开发者协作通道 | 不存在 | 实现任务级临时管道 | 不做自主修改平台代码的 agent |
...
8. 一个使用真实 Paperless-ngx 与 InvenTree 的复杂企业工作流实验。
...
- 不在本阶段扩展 Home Assistant WebSocket、ThingsBoard MQTT 或 Actual SDK/CLI；它们仍服从后续阶段顺序。
...
| 莉莉丝 | 题包公开部分、任务工作区、平台黑箱合同、自己的会话和工具结果、管道回复 | 搭建/修改/运行/验收工作流，提交报告，决定是否恢复 | 平台源码、平台 DB、隐藏 oracle、Codex 工作区 |
| 平台 | 工作流和运行事实、莉莉丝连接元数据、报告和审批状态 | 执行黑箱 API、持久化事件、实施权限门禁 | 代替莉莉丝决定最终工作流 |
| Codex | 平台源码、题包公开部分、环境、经路由允许的报告、开发测试 | 修题包、修环境、修通用平台能力、回传证据 | 未审批能力报告、隐藏成最终工作流、伪造莉莉丝反馈 |
...
- 她是本地运行的工作流 Builder 智能体，不是整个平台。
- 她的主要客户是需要真实 AI 化的传统企业，也允许单独报告个人/小团队泛化。
- 她通过平台公开积木、运行和验收接口交付工作流。
- 平台对她是黑箱；发现能力不足时不能猜测内部实现。
- 工作流设计错误由她自行修复。
- 正式实验中题目缺失和环境问题由出题方承担。
- 她不能把结构验收、mock 或自己的声明当作真实业务结果。

合作管道说明不得写入基础系统上下文。只有 assignment 在线路上显式携带 `collaboration` 时，daemon 才追加一段任务级临时说明并注册管道工具。普通 assignment 使用 `exclude_none` 投影，在线路数据、模型上下文和工具 schema 中完全省略该字段；会话归档必须能证明普通会话从未看见字段、说明或工具。
...
新增全局开发入口 `/developer/collaboration`。它属于 Developer Studio，不放入 Customer Runtime，也不塞进某个应用的普通运行页。

桌面布局：

| 区域 | 固定内容 |
| --- | --- |
| 左栏 | 题目、revision、莉莉丝会话、channel 状态、当前阻塞、未读数 |
| 中栏 | 按 seq 排列的需求、消息、工具调用、报告、审批、Codex 回复、复验 timeline |
| 右栏 | 当前报告完整证据、状态、路由、审批操作、关联草稿/run/Trace |
| 顶栏 | daemon 连接、题目 approval mode、预算、deadline、停止实验 |
...
这些测试只证明协作基础设施，不进入 `EXP-LILIES-001` 产品分母。
...
资格测试必须包括属性测试或循环故障注入，至少重复 100 次 idempotency/reconnect 场景，不能只测一条 happy path。
...
### 20.7 `V04-13-T01G`：管道资格测试

| 字段 | 内容 |
| --- | --- |
| 来源意图 | 第 19 节 |
| 依赖 | T01A 至 T01F |
| 具体改动 | 确定性测试、并发/断线故障注入、浏览器验收 |
| 禁止捷径 | 不把这些测试计入平台业务成功率 |
| 验收 | Q01-Q23 全部通过，100 次重放零丢失/零重复副作用 |
| 证据 | machine-readable qualification bundle |

### 20.8 `V04-13-T01H`：真实企业实验与动态平台补能

| 字段 | 内容 |
| --- | --- |
| 来源意图 | `LILIES-I002` 至 `I016`、`PRODUCT-015`、`PRODUCT-016` |
| 依赖 | T01G |
| 具体改动 | 冻结 EXP-LILIES-001 题包；真实启动；处理经用户批准的通用能力报告；持续同一上下文重试 |
| 动态缺口 ID | 每个获批报告生成 `V04-13-GAP-<report-short-id>`，验收来自报告的 missing_contract |
| Codex 边界 | 只能实现通用能力并回传证据；不得代写工作流或项目专用 adapter |
| 禁止捷径 | 不预先根据当前积木缺口偷跑实现；先由莉莉丝实际发现并经用户批准 |
| 验收 | 第 18.6 节真实 oracle；若预算内未通过，结果必须如实 fail |
| 证据 | 完整题包、会话、报告、diff、runs、oracle 和指标包 |
...
| --- | --- |
| 来源 report | 必填且已获批 |
| 通用能力合同 | 直接复制获批 missing_contract |
| 影响范围 | 不得含 Paperless/InvenTree 项目名分支 |
| 实现方案 | 由 Codex形成并在用户监控中可见 |
| 回归 | 通用单元测试 + 原复现 + 至少一个非题目项目/合约样本 |
...
| 证据 | closure review、release gate JSON、archive digest、提交 SHA |
...
正式实验启动脚本显式开启前两项。即使全局开启 collaboration，普通 assignment 没有 task-level access 时仍不得注册管道工具。
...
本文没有留给实施者选择的开放产品决策。以下默认已锁定，用户可在审批时修改：

- 本地 `lilies serve` 为成熟运行核心，CLI 和平台都是客户端。
- 首版使用 HTTP 写入、SSE 读取、持久 cursor/ack，不使用 WebSocket。
- 审批模式按题设置，默认 manual，新题重置。
- 平台能力报告必须用户批准；题包/环境报告直接给出题方。
