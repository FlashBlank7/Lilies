...
## 9. 大型集群实验(2026-08,8-agent 并行集群)

**实验**:8 个并行 agent 分析"制造企业出海东南亚"的 8 个方面(市场/渠道/合规/供应链/本地化/竞争/风险/成本),各自新鲜上下文并行,合成 agent 聚合为综合战略报告。

**结果**:
- ✅ **机制验证**(脚本 provider):8 个 agent 真并行(asyncio.gather + Semaphore),agent.started/completed 各 8,按名聚合,合成报告。
- ✅ **真实模型(DeepSeek)验证**:真实模型产出了完整出海战略报告(三阶段战略/渠道/供应链本地化/风险管控)。证明大型并行集群在宽度任务上真实可工作。
- 🔧 **实验暴露并修复两个真实 bug**:
  1. `parallel_agents` try/finally 中 result 未初始化 → 异常被 UnboundLocalError 掩盖(已修)。
  2. **空 tools 默认给全部工具** → 真实模型循环 tool_use 直到 max_turns,agent 循环不终止(已修:未配置工具时显式禁止,子 agent 默认无工具的纯推理者)。

**理论含义**:实验证实了"三个皮匠"的成立面——对宽度(可分解)任务,大型并行集群真实超出单体(上下文并行、吞吐);这正是有界涌现值得实现的原因。

**经验**:真实模型会暴露脚本 provider 掩盖的集成问题(工具默认值、终止判定)。**机制测试 + 真实模型测试都应纳入层级验证。**
