...
| P1-4 | 用真实 Builder 搭建上述三个场景(验证 Lilies 自动搭建对客户场景有效) | E2 | ✅ 已通过(2026-08):真实 Builder 自主搭建数据清洗工作流(start/record_collection_normalize/record_deduplicate/variable_assigner/end 全覆盖并发布);注:用测试输入运行该 Builder 生成草稿时,因测试输入名与 Builder 生成 start schema 不匹配而失败(非 Builder 缺陷) |

**验收状态(2026-08)**:三个场景 E2 证据已记录(见 examples/p1-scenarios/);P1-4 Builder 自主搭建已验证。**P1-1 数据管道已升级**(新增 file_read 积木,见下)。
