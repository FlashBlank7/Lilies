...
\begin{axiom}[Pair 幂等]
$\pairMonad^2 \cong \pairMonad$。将 Pair 分解再次应用于自身，不会发现新的分解——第一次分解已是最大分解。本公理不能从公理 1--2 导出，而是 Lilies 工程实践中观察到的结构不变量：在 200+ 个阶段的演化中，从未出现过需要第三层 Harness+LLM 嵌套的情况。
...
Yoneda 引理：对于任意范畴 $\cat{C}$ 和对象 $A \in \cat{C}$，$\homset{\cat{C}}{-}{A}$ 的自然变换与 $\homset{\cat{C}}{A}{-}$ 的元素一一对应。
...
\begin{proof}
每次重试独立的成功概率为 $p_{\mathsf{succ}} = 1 - p_{\mathsf{fail}} > 0$。$n$ 次尝试内成功的概率为 $1 - p_{\mathsf{fail}}^n \to 1$（当 $n \to \infty$）。因此 colimit 存在。
