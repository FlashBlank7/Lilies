...
本项目当前只有一位开发者。每次设计和实施都必须以最小、可运行、可验证、可由单人审查的核心闭环为目标：

1. 严格守住当前用户指令、任务合同和验收边界，不在实现过程中自行扩展相邻功能或未来框架。
2. 先完成并验证核心功能，再考虑复用、抽象、发布、打包、配套界面和额外治理。
3. 新发现的工作只有在严格阻塞当前验收时才进入当前任务，否则不做。
4. 选择满足验收的最简单可用实现；复杂度只能由真实失败或明确需求推动。
5. 不得用最小可用原则降低验收、交付空壳或假装完成；它用于防止范围膨胀、半途而废和单人无法审查的缺陷面。
